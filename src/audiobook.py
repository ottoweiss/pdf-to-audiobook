...
def play_split_and_combine(index, text, voice, speed):
    client = openai.OpenAI(api_key=KEY)
    combined_audio = AudioSegment.silent(duration=0)
    segments = split_at_sentence_boundary(text)
    for segment in segments:
        response = client.audio.speech.create(model="tts-1", voice=voice, input=segment, speed=speed)
        byte_stream = io.BytesIO(response.content)
        audio = AudioSegment.from_file(byte_stream, format="mp3")
        combined_audio += audio
    return index, combined_audio

def get_audiobook(json_file, book_title="audiobook", voice="onyx", speed="1.0"):
    book_directory = f"{book_title}"
